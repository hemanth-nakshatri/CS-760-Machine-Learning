...
class A2C(AbstractSolver):
    ...
    def train(self, states, actions, rewards, next_state, done):
        ...
        # returns[-1] = rewards[-1] + (1 - done) * self.options.gamma * self.actor_critic.value(torch.tensor(next_state, dtype=torch.float32))
        # returns = torch.tensor(returns, dtype=torch.float32)
        current_return = 0
        # if not done:
        #     current_return = self.actor_critic.value(torch.tensor(next_state, dtype=torch.float32)).detach().numpy()
        
        for t in reversed(range(len(rewards))):
            current_return = rewards[t] + (1-done) * self.options.gamma * self.actor_critic.value(torch.tensor(next_state, dtype=torch.float32))
            # current_return = rewards[t] + self.options.gamma * current_return
        ...
        ################################
        #   YOUR IMPLEMENTATION HERE   #
        policy_loss = -(log_probs * advantages)  # Note the negative sign
